 ...
 * narrows. Receipts and Claims always keep their labels. Reduced role tabs —
 * whether compressed to an icon in the row or overflowed out of it — are ALSO
 * listed with labels in the ONE account menu (not a second dropdown), and a
 * badge hidden from the row re-surfaces on the avatar. We stub the badges
 * endpoint so the four functional tabs are present regardless of the e-sign
 * backend state.
 */
test("nav keeps Receipts/Claims labels and lists reduced role tabs in the account menu", async ({
  ...
  // Wide: all four tabs in the row with labels; nothing reduced into the menu.
  ...
  // and/or overflowed) both appear — with labels — in the account menu.
