...
## Register for notifications & consent text

All `Notifications.*` strings (push bodies, the soft-ask, settings card) use
the FORMAL second person in both Chinese catalogs — 您, never 你. Lock-screen
text addressed to elders keeps a consistent, respectful register
(docs/NOTIFICATIONS_DESIGN.md §9.1).

OS and browser chrome quoted in instructions ("Allow", "Add to Home Screen",
"Site settings") is NOT naturalized: keep the English label, optionally with
the Chinese beside it — the user must be able to match the words on their
screen (§8.3).
