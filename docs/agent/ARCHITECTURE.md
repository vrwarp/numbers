...
| `/api/reimbursements/[id]/revert` | POST | generated only (409 else); claim → draft, its receipts → unassigned, AuditEvent(revert-to-draft). Rows keep isVerified (values were frozen; edits still revoke) |
