...
| `TIME_ZONE` | IANA zone human-facing calendar dates are computed in (`appTimeZone()`; "Today"/"Yesterday" labels, the MM/DD/YYYY stamped on claim packets + approval marks, admin usage-chart day buckets) — carried to the client as next-intl's global `timeZone`. Default/invalid → `America/Los_Angeles` (Hayward, CA). Admin-editable (Settings → Deployment) |
