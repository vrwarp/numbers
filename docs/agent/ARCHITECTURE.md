...
| `/api/mcp` (the `[transport]` dynamic route, basePath `/api`) | GET POST DELETE | **MCP backend** (Streamable HTTP, stateless; docs/MCP_DESIGN.md). The ONE route NOT on the session cookie: `withMcpAuth` → `verifyMcpToken` resolves a **Bearer personal access token** to `{userId, scopes}` (401 else). Owner-scoped, per-tool scope gate. 14 tools — read receipts/claims/ministries, create/edit DRAFT claims + line items (never sets `isVerified`), list teams/positions, stage/list/discard catalog-edit drafts. No sign/submit/approve/pay/pdf/verify, and **no tool calls an LLM** (no suggestion tool, no `extractWithAi` — draft-building consumes stored annotations only, so the backend never spends AI quota). Every draft-producing tool also returns an absolute `approvalUrl` (the `/claims/[id]` review page or `/catalog-drafts`) for the assistant to hand the user. Write tools reuse the same service fns as the REST routes (`src/lib/claims.ts`, `claim-edits.ts`) |
