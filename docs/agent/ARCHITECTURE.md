...
src/lib/positions.ts            Positions (custom approval roles): approverEligibility +
                                ...
                                seed + empty-table fallback) — dependency-free, unit-tested, client-safe
src/lib/positions-catalog.ts    Position table reads (empty table → DEFAULT_POSITION_ENTRIES, id:null,
                                like loadAllMinistryRows) + resolveSuggestedApprover(claim) (SERVER);
