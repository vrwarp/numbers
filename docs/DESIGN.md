...
| 4 | One LLM call per receipt, at claim time | One batched call per claim | Batching confused small vision models — items got attributed to the wrong receipt. Per-receipt calls make attribution exact by construction (the server stamps the id), and at ~$0.001/receipt the extra cost is noise. Extraction originally waited for claim time so unclaimed receipts cost nothing — the *when* is superseded by #12; the one-call-per-receipt shape stands. |
